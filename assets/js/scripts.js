...
if (search_field) {
    let no_items = `<li><span class="block text-gray-500 py-3 px-2.5 truncate">Enter a search term to find results.</span></li>`;
        ...
        Array.prototype.forEach.call(resp, data => {
            if (data) {
                if (data.page !== data.title) {
                    ...
                    result += `<li>
                                   <a class="block hover:bg-gray-100 py-1.5 px-2.5 text-gray-800 hover:text-gray-700 truncate" href="${data.link}">${data.title}</a>
                               </li>`;
                }
            }
        });
